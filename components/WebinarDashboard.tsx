...
const DashboardComponent = () => {
  ...
  const [showSignInMessage, setShowSignInMessage] = useState(false); // To control message visibility
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (status === "authenticated" && session?.user?.email) {
        ...
        setShowSignInMessage(true); // Show sign-in message when not authenticated
        setLoading(false); // Stop loading
      }
    };
    ...
    <div className="p-4 max-w-6xl mx-auto">
      ...
      {showSignInMessage && (
        <div className="bg-yellow-100 text-yellow-800 p-4 rounded-lg mb-6">
          <p className="text-lg font-semibold">You need to sign in before you can view your dashboard.</p>
        </div>
      )}
